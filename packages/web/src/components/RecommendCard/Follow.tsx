...
const Follow = ({ profile, setFollowed, followed }: Props) => {
    ...
    return (
        <button
            className='flex justify-center items-center rounded-full px-4 py-2 text-primary-blue bg-black border border-primary-blue font-semibold h-10 hover:bg-opacity-10 disabled:bg-opacity-10'
            onClick={createFollow}
            disabled={
                typedDataLoading || signLoading || writeLoading || broadcastLoading || followed
            }>
            {(typedDataLoading || signLoading || writeLoading || broadcastLoading) && <Spinner className='mr-2 border-t-black border-primary-blue' size='xs' />}
